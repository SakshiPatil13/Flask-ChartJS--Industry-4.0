...
def flask_upload(machine_number, cycle_time, running_status, status, running_hours,
                 downtime_hours, pieces, total_pieces, parameter1,
                 parameter2, parameter3):

    data = {'machine_number': machine_number, 'cycle_time': cycle_time, 'running_status': running_status,
...
while True:
    for machine in range(1, 4):
        ...
        if running[0] == 1:
            ...
            cycle = random.sample(range(1, 8), 1)
            print 'Cycle Time: ', cycle[0]
        ...
        flask_upload(machine, cycle[0], running[0], status, running_hours[machine - 1],
